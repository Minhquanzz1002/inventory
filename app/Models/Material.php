...
class Material extends Model
{
    ...
    public function validate(array $data, bool $isUpdate = false) : string
    {
        $validators = [
            ...
            'origin' => v::notEmpty()->setName('origin')->setTemplate('Xuất xứ không hợp lệ. Xuất xứ không được để trống và viết hoa chữ cái đầu.'),
            'status' => v::notEmpty()->in(['IN_STOCK','OUT_OF_STOCK','TEMPORARILY_SUSPENDED'])->setName('status')->setTemplate('Trạng thái không hợp lệ. Trạng thái chỉ có thể là IN_STOCK, TEMPORARILY_SUSPENDED hoặc OUT_OF_STOCK.'),
